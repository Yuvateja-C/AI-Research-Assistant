  ...
  const handleCreateReport = async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        setReportError(d.detail || "Failed to generate report");
      ...
      <main role="main" style={{ flex:1, display:"flex", flexDirection:"column", height:"100%", position:"relative", zIndex:5, overflow:"hidden" }}>
            ...
            <div>
              <h1 onClick={() => { setActivePage("workspace"); setActiveTab("workspace"); }} style={{ fontSize:15, fontWeight:700, color:"var(--text)", letterSpacing:"-0.01em", lineHeight:1.2, cursor:"pointer" }}>
                {activePage === "workspace" ? (
                  ...
                  activePage === "refund" ? "Cancellation & Refund Policy" :
                  activePage === "cookie" ? "Cookie Policy" :
                  activePage === "about" ? "About Us" :
                  activePage === "disclaimer" ? "Disclaimer" : "Contact Us"
                )}
              </h1>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"12px 0", display:"flex", flexDirection:"column" }}>
          <div style={{ maxWidth:800, margin:"0 auto", padding:"0 16px", display:"flex", flexDirection:"column", minHeight:"100%", width:"100%" }}>
            ...
            {activePage === "cookie" && (
              <div className="anim-scaleIn" style={{ padding:"24px 0", color:"var(--text-2)" }}>
                <h2 style={{ color:"var(--text)", marginBottom:16 }}>Cookie Policy</h2>
                <p style={{ marginBottom:14 }}>Effective Date: June 27, 2026</p>
                <p style={{ marginBottom:14 }}>ResearchAI uses cookies and similar tracking technologies to enhance your workspace experience.</p>
                <h3 style={{ color:"var(--text)", marginTop:20, marginBottom:8 }}>1. What are cookies?</h3>
                <p style={{ marginBottom:14 }}>Cookies are small text files placed on your device to store data that can be recalled by a web server in the domain that placed the cookie.</p>
                <h3 style={{ color:"var(--text)", marginTop:20, marginBottom:8 }}>2. How we use cookies</h3>
                <p style={{ marginBottom:14 }}>We use cookies to maintain your login session state securely (via secure, http-only session tokens) and to store user preferences such as your choice of theme (dark or light mode).</p>
                <button onClick={() => setActivePage("workspace")} style={{ padding:"10px 20px", marginTop:24, borderRadius:8, background:"var(--grad)", border:"none", color:"#fff", fontWeight:"bold", cursor:"pointer" }}>Back to Workspace</button>
              </div>
            )}
            ...
            {activePage === "about" && (
              <div className="anim-scaleIn" style={{ padding:"24px 0", color:"var(--text-2)" }}>
                <h2 style={{ color:"var(--text)", marginBottom:16 }}>About Us</h2>
                <p style={{ marginBottom:14 }}>ResearchAI is a modern, state-of-the-art document intelligence platform designed to empower researchers, engineers, and data analysts.</p>
                <p style={{ marginBottom:14 }}>By utilizing advanced vector embeddings and Retrieval-Augmented Generation (RAG), we allow users to query massive documents instantly and compile professional reports with executive summaries, recommendations, and clear methodologies in seconds.</p>
                <p style={{ marginBottom:14 }}>Our mission is to accelerate the cycle of research and technical analysis, making it easy for users to get from raw documents to actionable insights.</p>
                <button onClick={() => setActivePage("workspace")} style={{ padding:"10px 20px", marginTop:24, borderRadius:8, background:"var(--grad)", border:"none", color:"#fff", fontWeight:"bold", cursor:"pointer" }}>Back to Workspace</button>
              </div>
            )}
            ...
            {activePage === "disclaimer" && (
              <div className="anim-scaleIn" style={{ padding:"24px 0", color:"var(--text-2)" }}>
                <h2 style={{ color:"var(--text)", marginBottom:16 }}>Disclaimer</h2>
                <p style={{ marginBottom:14 }}>Effective Date: June 27, 2026</p>
                <p style={{ marginBottom:14 }}>The information generated by the ResearchAI platform is compiled automatically using advanced artificial intelligence models. This output is intended solely for general reference and informational purposes.</p>
                <h3 style={{ color:"var(--text)", marginTop:20, marginBottom:8 }}>1. No Professional Advice</h3>
                <p style={{ marginBottom:14 }}>ResearchAI does not provide medical, legal, financial, or academic certification advice. Any research output should be verified by a qualified professional before making business-critical decisions.</p>
                <h3 style={{ color:"var(--text)", marginTop:20, marginBottom:8 }}>2. Accuracy Limitations</h3>
                <p style={{ marginBottom:14 }}>Retrieval-augmented outputs are dependent on the completeness and accuracy of user-uploaded text source documents. We do not assume responsibility for errors, omissions, or hallucinations in the LLM-generated reports.</p>
                <button onClick={() => setActivePage("workspace")} style={{ padding:"10px 20px", marginTop:24, borderRadius:8, background:"var(--grad)", border:"none", color:"#fff", fontWeight:"bold", cursor:"pointer" }}>Back to Workspace</button>
              </div>
            )}
        ...
        <div style={{ padding:"8px 16px 16px", paddingBottom: isMobile ? "max(16px, env(safe-area-inset-bottom))" : 16 }}>
          ...
          <footer role="contentinfo" style={{ display:"flex", gap:14, justifyContent:"center", alignItems:"center", marginTop:12, flexWrap:"wrap" }}>
            ...
            <span style={{ color:"var(--text-3)" }}>·</span>
            <span onClick={() => setActivePage("cookie")} style={{ fontSize:11, color:"var(--accent)", cursor:"pointer", textDecoration:"underline" }}>Cookie Policy</span>
            <span style={{ color:"var(--text-3)" }}>·</span>
            <span onClick={() => setActivePage("about")} style={{ fontSize:11, color:"var(--accent)", cursor:"pointer", textDecoration:"underline" }}>About Us</span>
            <span style={{ color:"var(--text-3)" }}>·</span>
            <span onClick={() => setActivePage("disclaimer")} style={{ fontSize:11, color:"var(--accent)", cursor:"pointer", textDecoration:"underline" }}>Disclaimer</span>
            <span style={{ color:"var(--text-3)" }}>·</span>
            <span onClick={() => setActivePage("contact")} style={{ fontSize:11, color:"var(--accent)", cursor:"pointer", textDecoration:"underline" }}>Contact Support</span>
          </footer>
        </div>
      </main>

      {/* Floating Toast Notification Container */}
      <div style={{ position:"fixed", bottom:24, right:24, zIndex:10000, display:"flex", flexDirection:"column", gap:8, maxWidth:320 }}>
        {toasts.map(t => (
          <div key={t.id} className="anim-scaleIn" style={{
            padding:"12px 18px", borderRadius:12, background:"rgba(20,20,28,0.9)", backdropFilter:"blur(12px)",
            border:`1px solid ${t.type === 'error' ? 'rgba(239,68,68,0.2)' : 'rgba(124,91,245,0.2)'}`,
            boxShadow:"0 10px 30px rgba(0,0,0,0.35)", display:"flex", alignItems:"center", gap:10
          }}>
            <span style={{ color: t.type === "error" ? "var(--red)" : "var(--accent)", fontSize:14 }}>
              {t.type === "error" ? "⚠️" : "✓"}
            </span>
            <span style={{ fontSize:12.5, fontWeight:600, color:"#fff" }}>{t.message}</span>
          </div>
        ))}
      </div>
